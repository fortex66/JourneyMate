        ...
        const updatedItems = await Promise.all(
            getlist.map(async (item) => {
                ...
                if (matchingUserCount) {
                    item.dataValues.group_chatting.dataValues.userCount =
                        matchingUserCount.dataValues.userCount;
                } else {
    ...
    try {
        ...
        if (enter) {
            const chatMessage = await chat.Message.findAndCountAll({
                ...
                where: {
                    chatID: chatID,
                    sendtime: { [Op.gte]: enter.enterTime },
                },
            });
            res.status(200).json({
                chatMessage,
...
const uploadImage = (io) => async (req, res) => {
    ...
    if (target) {
        target.broadcast.to(roomID).emit("chat_message", {
            roomID: roomID,
            userID: userID,
