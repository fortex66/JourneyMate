...
async function uploadpost(req, res) {
  ...
  try {
    ...
    // 요청 데이터로 받은 게시물 정보를 travel_post 테이블에 저장합니다.
...
async function deletepost(req, res) {
  ...
  try {
    const result = await tUpload.tPost.destroy({
      where: { tpostID: param /*, userID:req.body.userID */ },
    }); // postID 파라메터와 같은 게시물 삭제
    console.log(result);
...
async function updatePost(req, res) {
  ...
  try {
    ...
    if (user !== existedPost.userID) {
      ...
      await tUpload.tPostImage.destroy({
        where: { tpostID: tpostID /*, userID:req.body.userID */ },
      }); // 기존 게시물의 사진을 삭제
...
async function companionDeletepost(req, res) {
  ...
  try {
    const result = await cUpload.cPost.destroy({
      where: { cpostID: param /*, userID:req.body.userID */ },
    }); // postID 파라메터와 같은 게시물 삭제
    console.log(result);
    res
      .status(200)
...
async function companionUpdatePost(req, res) {
  try {
    ...
    if (user !== existedPost.userID) {
      ...
      await cUpload.cPostImage.destroy({
        where: { cpostID: cpostID /*, userID:req.body.userID */ },
      }); // 기존 게시물의 사진을 삭제
