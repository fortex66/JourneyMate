...
const ChattingRoom = () => {
    ...
    useEffect(() => {
        ...
        const fetchChatRoom = async () => {
            const response = await axios.get(
                baseURL + `chat/chatroomdata/${chatID}`
            );
            ...
            reader.onload = async () => {
                ...
                try {
                    ...
                    const result = await axios.post(
                        baseURL + `chat/chattingRoom/${chatID}`,
                        formData,
                        {
                            headers: {
                    ...
                    setMessages([
                        ...messages,
                        {
                            text: result.data.saveMessage.content,
                            self: true,
                            userID: currentUser,
                            messageType: 1,
                        },
        ...
        newFileInput.addEventListener("change", (e) => {
            ...
            reader.onload = async () => {
                ...
                try {
                    // 서버로 게시물ID, file, i 보내기
                    const result = await axios.post(
                        baseURL + `chat/chattingRoom/video/${chatID}`,
                        formData,
                        {
                            headers: {
                                "Content-Type": "multipart/form-data", // multipart/form-data로 보낸다고 명시
        ...
        newFileInput.addEventListener("change", (e) => {
            ...
            reader.onload = async () => {
                ...
                try {
                    // 서버로 게시물ID, file, i 보내기
                    const result = await axios.post(
                        baseURL + `chat/chattingRoom/file/${chatID}`,
                        formData,
                        {
                            headers: {
                                "Content-Type": "multipart/form-data", // multipart/form-data로 보낸다고 명시
    ...
    async function download(imageUrl) {
        await axios
            .get(`${baseURL}chat/download`, { params: { key: imageUrl } })
            .then((response) => {
            ...
            <TopContainer>
                <Header>
                    ...
                    <TitleContainer>
                        <Title>
                            {roomdata &&
                                roomdata.data.chatRoomData.companion_post.title}
                        </Title>
                        <FontAwesomeIcon
                            icon={faUser}
                            size="1x"
                ...
                {messageData &&
                    [...messageData.data.chatMessage.rows]
                        ...
                        .map((prevchat, index) => {
                            ...
                            const profileImage =
                                matchedUser?.User?.profileImage;
                            const isCurrentUser =
                                currentUser === prevchat.userID; // 현재 사용자와 이전의 채팅 userID가 일치하는지 확인
                                        ...
                                        <MessageContainer>
                                            ...
                                            {prevchat.messageType === 2 && (
                                                <VideoContainer>
                                                    <video controls>
                                                        <source
                                                            src={`${imgURL}${prevchat.content.replace(
                                                                /\\/g,
                                                                "/"
                                                            )}`}
                                                            type="video/mp4"
                                ...
                                <ChatContainer key={index} self={message.self}>
                                    ...
                                    <MessageContainer>
                                        ...
                                        {message.messageType === 1 && (
                                            <ImageContainer>
                                                <img
                                                    src={`${imgURL}${message.text.replace(
                                                        /\\/g,
                                                        "/"
                                                    )}`}
                                                    alt="Chat image"
